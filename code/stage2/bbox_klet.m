function output=bbox_klet(a,poslist,first_el,last_el)

%% BBOX_KLET computes the mean relative location of the bbox for each klet
% Input
% a: annotation list corresponding to the poslist of the triplets
% poslist: Kx1 cell array of the poslist for each part k=1...K
% Output
% output: struct of kps_mean [Kpx2xKxN array of the mean relative location of Kp keypoints for
% each of the K parts of each of the N klets] and kps_weights [NxKp]
...
for n=first_el:last_el
    ...
    for k=1:K
        ...
        for i=1:part.size
            ...
            width = bounds(3);
            coords = [a.bounds(part.dst_entry_ids(i),1:2); a.bounds(part.dst_entry_ids(i),1:2)+a.bounds(part.dst_entry_ids(i),3:4)];%a.coords(:,1:2,part.dst_entry_ids(i));
...
errs_std = nan(Kp,2,last_el-first_el+1);
...
for n=first_el:last_el
    ...
    for i=1:poslist{1}(n).size
		dst_kps(:,:,i) = [a.bounds(poslist{1}(n).dst_entry_ids(i),1:2); a.bounds(poslist{1}(n).dst_entry_ids(i),1:2)+a.bounds(poslist{1}(n).dst_entry_ids(i),3:4)];%
    ...
    for kp = 1:Kp
        ...
        if sum(H(:))==0, 
            ...
            alphas = quadprog(2*H,f,[],[],Aeq,beq,zeros(K,1),ones(K,1),[],opts);
        end
